/*
Wherefore art thou
Make a function that looks through an array of objects (first argument) and 
returns an array of all objects that have matching name and value pairs (second argument). 
Each name and value pair of the source object has to be present in the object from the collection 
if it is to be included in the returned array.

For example, if the first argument is [{ first: "Romeo", last: "Montague" }, { first: "Mercutio", last: null }, 
{ first: "Tybalt", last: "Capulet" }], and the second argument is { last: "Capulet" }, 
then you must return the third object from the array (the first argument), 
because it contains the name and its value, that was passed on as the second argument.
*/
function whatIsInAName(collection, source) {
  const keyArr = Object.keys(source)
  let match = 0
  let resultArr = []
  for (let i = 0; i < collection.length; i++) {
    for (let j = 0; j < keyArr.length; j++) {
      if (
        collection[i].hasOwnProperty(keyArr[j]) &&
        collection[i][keyArr[j]] == source[keyArr[j]]
      ) {
        console.log("currently checking", collection[i])
        match++
      }
    }
    if (match == keyArr.length) {
      // console.log("Found match", match, collection[i])
      resultArr.push(collection[i])
      match = 0
    } else {
      match = 0
    }
  }
  return resultArr
}
// const result = whatIsInAName(
//   [
//     { first: "Romeo", last: "Montague" },
//     { first: "Mercutio", last: null },
//     { first: "Tybalt", last: "Capulet" },
//   ],
//   { last: "Capulet" }
// )
const result = whatIsInAName(
  [{ apple: 1, bat: 2 }, { apple: 1 }, { apple: 1, bat: 2, cookie: 2 }],
  { apple: 1, cookie: 2 }
)
console.log("Result: ", result)
